        ...
        var chances = [5, 2, 6, 3, 8, 10,
            ...
            var resourcesMap = {wood: {idx: 0, count: 4}, brick: {idx: 1, count: 3}, 
                ...
                var neighbors = [{x: 0, y: -1}, {x: 1, y: -1}, 
                    ...
                    $hexes.each(function(idx){
                        var $hex = $hexes.eq(idx);
                        ...
                        var $newBorder = $("<div/>").addClass("border north-west");
                        $newBorder.css("left", left - 5);
                        $newBorder.css("top", top + 3);
                        $newBorder.appendTo("body");

                        var $newBorder = $("<div/>").addClass("border north-east");
                        $newBorder.css("left", left - 5 + width - 47);
                        $newBorder.css("top", top + 3);
                        $newBorder.appendTo("body");

                        var $newBorder = $("<div/>").addClass("border west");
                        $newBorder.css("left", left - 5);
                        $newBorder.css("top", top + 3);
                        $newBorder.appendTo("body");
                        ...
                        if (y >= 0) {
                            // middle downward
                            if (x == -1 * radius || y == radius) {
                                // first hex on the row
                                // or a hex on the last row 
                                // - paint south-west
                                var $newBorder = $("<div/>").addClass("border south-west");
                                $newBorder.css("left", left - 5);
                                $newBorder.css("top", top + 3 + height - 20);
                                $newBorder.appendTo("body");
                            }
                            if (x == radius || y == radius || (x + y) == radius) {
                                // last hex on the row
                                // or a hex on the last row
                                // - paint south-east
                                var $newBorder = $("<div/>").addClass("border south-east");
                                $newBorder.css("left", left - 5 + width - 47);
                                $newBorder.css("top", top + 3 + height - 20);
                                $newBorder.appendTo("body");
                            }
                        ...
                        if (x == radius || (x + y) == radius) {
                            var $newBorder = $("<div/>").addClass("border east");
                            $newBorder.css("left", left - 5 + width - 47);
                            $newBorder.css("top", top + 3);
                            $newBorder.appendTo("body");
                        }
                        
                        var $newSettlement = $("<div/>").addClass("settlement north-west");
                        $newSettlement.css("left", left - 12);
                        $newSettlement.css("top", top + 18);
                        $newSettlement.appendTo("body");

                        var $newSettlement = $("<div/>").addClass("settlement south-west");
                        $newSettlement.css("left", left - 12);
                        $newSettlement.css("top", top + 3 + height - 35);
                        $newSettlement.appendTo("body");
                       
                        if (y == -1 * radius) {
                            // first row
                            var $newSettlement = $("<div/>").addClass("settlement north");
                            $newSettlement.css("left", left - 8 + (width / 2));
                            $newSettlement.css("top", top - 10);
                            $newSettlement.appendTo("body");
                        }
                        
                        if (y == radius || (x + y) == radius) {
                            // last row or last in the second half of rows
                            var $newSettlement = $("<div/>").addClass("settlement south");
                            $newSettlement.css("left", left - 8 + (width / 2));
                            $newSettlement.css("top", top + 3 + height - 5);
                            $newSettlement.appendTo("body");
                        }

                        if (x == radius) {
                            // last hex on the first half of rows
                            var $newSettlement = $("<div/>").addClass("settlement north-east");
                            $newSettlement.css("left", left - 5 + width + 2);
                            $newSettlement.css("top", top + 18);
                            $newSettlement.appendTo("body");
                        }
                        if (x == radius || ((x + y) == radius && x != 2)) {
                            var $newSettlement = $("<div/>").addClass("settlement south-east");
                            $newSettlement.css("left", left - 5 + width + 2);
                            $newSettlement.css("top", top + 3 + height - 35);
                            $newSettlement.appendTo("body");
                        }
                    ...
                    $settlements.hover(function(){
                        var $settlement = $(this);
                        $settlements.removeClass("hover");
                        $settlement.addClass("hover");
                    }, function(){
                        $settlements.removeClass("hover");
                        $settlement.removeClass("hover");
                    });
});
